...
func (r *iexec) podPrompt(matchingPods []v1.Pod) error {
	fmt.Println("Filter:", r.search)
	if len(matchingPods) == 1 {
		r.pod = matchingPods[0]
		r.namespace = r.pod.GetNamespace()
		fmt.Println("Match Pod:", r.pod.Name)
		return nil
	}

	templates := &promptui.SelectTemplates{
		// Active:   fmt.Sprintf("Namespace: {{ .Namespace | blue }} | Pod: %s {{ .Name | cyan }} | {{.Status.Phase | cyan }}", promptui.IconSelect),
		Active:   fmt.Sprintf("Pod : {{ .Namespace | blue }}/{{ .Name | cyan }} | {{.Status.Phase | cyan }} %s", promptui.IconSelect),
...
func (r *iexec) do() error {
	...
	fmt.Println("\n\nWelcome to use kubectl-iexec!")
	fmt.Println("More info: https://github.com/airdb/kubectl-iexec.")
	err = r.exec()
	if err != nil {
		return err
	}
